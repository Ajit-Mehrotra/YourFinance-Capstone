...
class PlaidController extends Controller
{
    public function createLinkToken()
    {
        ...
        // ----- hardcoded user_id for now, but should be dynamic ------
        $user_id = 1; 
        ...
        try {
            DB::transaction(function () use ($plaid, $account) {
                try {
                    ...
                    $response = json_decode(json_encode($e->getMessage(), true), true);
            ...
            Log::error('Adding holdings failed: ' . json_encode($e->getMessage()));
            return [
                'result' => 'error',
                'message' => $e
            ];
